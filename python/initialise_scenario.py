...
def init_scenario(speed: float, n_cars: int = 5, n_customers: int = 10):
    ...
    url1 = f"http://{host}:8080/scenario/create"
    ...
    url2 = f"http://{host}:8090/Scenarios/initialize_scenario?db_scenario_id={scenario.id}"
    response = requests.post(url2, json={})
    print("Scenario initialized successfully.") if response.status_code == 200 else None

    url3 = f"http://{host}:8090/Runner/launch_scenario/{scenario.id}?speed={speed}"
